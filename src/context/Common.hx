...
typedef SharedDisplayInformation = {
	import_positions : PMap<core.Globals.Pos, {b:ocaml.Ref<Bool>, l:ImmutableList<core.Ast.PlacedName>}>,
...
class Context {
	...
	public var package_rules : PMap<String, PackageRule>;
	...
	public var cached_macros : Hashtbl<
	...
	>;
	public var stored_typed_exprs : PMap<Int, core.Type.TExpr>;
	// output
	public var file : String;
	public var flash_version : Float;
	public var features : Hashtbl<String, Bool>;
	...
	public var net_path_map : Hashtbl<core.Path, {
...
class Common {
	...
	public static function create(version:Int, s_version:Void->String, args:ImmutableList<String>) : Context {
		...
		var defines:PMap<String, String>= PMap.empty();
		defines = PMap.add("true", "1", defines);
		defines = PMap.add("source-header", "Generated by Haxe "+s_version(), defines);
		if (display_default.get() !=  DMNone) {
			defines = PMap.add("display","1", defines);
		}
		// let defines =
			// PMap.add "true" "1" (
		...
		var ctx:Context = {
			...
			verbose : false,
			foptimize : true,
			features : new Hashtbl<String, Bool>(),
			platform : Cross,
	...
	public static function clone (com:Context) : Context {
		...
		return com.with({
			...
			callbacks: create_callbacks(),
			display_information: {
				unresolved_identifiers: Tl,
				interface_field_implementations: Tl
			},
			defines: {
	...
	public static function get_config (com:Context) : Platform_config {
		var defined = function (f:core.Define.StrictDefined) : Bool {
			return PMap.exists(core.Define.infos(f).a, com.defines.values);
		};
